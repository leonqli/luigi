...
def _build_qsub_command(cmd, job_name, outfile, errfile, pe, n_cpu):
    ...
    qsub_template = """echo {cmd} | qsub -o ":{outfile}" -e ":{errfile}" -m e -l h_rt=28800,h_vmem=8G,mem_free=6G -V -r y -pe {pe} {n_cpu} -N {job_name}"""
    ...
    def _dump(self, out_dir=''):
        ...
        with self.no_unpicklable_properties():
            ...
            if self.__module__ == '__main__':
                ...
                #logger.debug("## args[0]: %s", sys.argv[0])
                #logger.debug("## module name: %s", module_name)
                #logger.debug("## before replace: %s", d)
                d = d.replace(b'c__main__', b"c" + module_name)
                #logger.debug("## replaced: %s", d)
